...
export const RssBuilder = (
  ...
  if (order == "reverse") {
    for (
      let i = items.length - 1 - startindex;
      i > items.length - 1 - endindex;
      i--
    ) {
      BuilderAlgo(i, items, container);
    }
  } else {
...
const BuilderAlgo = (i, items, container) => {
  ...
  title.appendChild(el.TextElement("h2", element.title["#cdata"]));
